        ...
        <div class="w-full h-full absolute top-0 left-0 bg-gray-800 bg-opacity-60 flex justify-center items-center">
            <div class="w-full p-8 flex justify-center items-left text-white" data-aos="zoom-in">
                <div class="w-full md:w-3/5 text-lg">
                    ...
                    <p class="mb-2 text-xl">{{ __('Münsters Wissensquartiere sind in Bewegung und neue Orte für Wissen, Wohnen, Leben und Arbeiten entstehen. Diese Veränderung muss heutigen Anforderungen an eine klimafreundliche und gemeinwohlorientierte aber auch soziale Stadt Münster gerecht werden.') }}</p>
                    <p class="mb-2 text-xl">{{ __('Der öffentliche Raum ist dabei ein wichtiger Grundpfeiler für eine lebenswerte und klimagerechte Stadt Münster. Bisher sind Straßen vor allem Zufarts-, Verbindungswege und Orte der Moilität. Sie können aber viel mehr sein.') }}</p>
                    <p class="mb-2 text-xl">{{ __('Das haben Studierende von FH und WWU in einem gemeinsamen Lehrprojekt auf Initative der Allianz für Wissenschaft Münster ein Semester lang erkundet und experimentiert.') }}</p>   
                    <p class="mb-2 text-xl">{{ __('In der Woche vom 29. September bis 3. Oktober präsentieren wir unsere Ergebnisse auf einem Teilstück der Corrensstraße. Laufen Sie schon jetzt virtuell über die Corrensstraße und entdecke Sie die unterschiedlichen Elemente eines nachhaltigen Straßenraums von morgen.') }}</p>
                    <a href="{{ route('map') }}">
                        <button class="py-2 px-3 text-xl mt-4 border rounded transition hover:bg-white hover:text-black">{{ __('Zur Übersichtskarte') }}</button>
                    </a>
                </div>
            </div>
        </div>
