...
function initMap() {
  var POLYGONS = {};
  var MARKERS = {}; //var markers = L.layerGroup([]);

  var mymap = L.map('map', {
    attributionControl: false,
    center: [51.968159, 7.596715],
    zoom: 17 //layers: [markers]
  ...
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    maxZoom: 22 //id: 'mapbox/streets-v11',
    //tileSize: 512,
    //zoomOffset: -1
    //accessToken: 'your.mapbox.access.token'

  }).addTo(mymap);
  var polygonColors = ['rgb(16, 185, 129)', 'rgb(59, 130, 246)', 'rgb(239, 68, 68)', 'rgb(245, 158, 11)', 'rgb(107, 114, 128)'];
  var polygonIndex = 0;
  fetch('api/polygon').then(function (response) {
  ...
  function addPolygon(data) {
    for (index in data.data) {
      ...
      var geojsonShape = L.geoJson(JSON.parse(polygon.geojson), {
        style: polystyle
      }).addTo(mymap);
      ...
      document.getElementById('card_polygon-' + polygon.id).onclick = function (view) {
        id = this.id.substr(this.id.lastIndexOf("-") + 1); // console.log(id);
  ...
  function addMarker(data) {
    for (index in data.data) {
      ...
      marker.bindPopup("<div class=\"flex items-center p-4 rounded-lg shadow-xs m-2\">\n" + "            <div>\n" + "                <p class=\"text-lg font-semibold\">\n" + suggestion.name + "                </p>\n<span class='text-base'>Für eine Augmented Reality Ansicht der Vorschläge den QR Code scannen...</span><br>\<a class='font-black' href='https://correnslab.de/ar'><img src='https://chart.apis.google.com/chart?chs=500x500&cht=qr&chld=L&chl=https://correnslab.de/ar'/><br>oder: <span class='text-lg hover:underline hover:font-bold hover:text-blue'>Hier klicken</span></a>" + "            </div>\n" + "            </div>", 1);
